...
export const surfacesCustomizations: Components<Theme> = {
  ...
  MuiDialog: {
    styleOverrides: {
      paper: ({ theme }: { theme: any }) => ({ backgroundColor: (theme.vars || theme).palette.background.paper }),
    },
  },
  MuiCard: {
    styleOverrides: {
      root: ({ theme }: { theme: any }) => ({
        padding: 16, gap: 16, transition: 'all 100ms ease', backgroundColor: gray[50],
        borderRadius: (theme.vars || theme).shape.borderRadius,
        border: `1px solid ${(theme.vars || theme).palette.divider}`, boxShadow: 'none',
        ...theme.applyStyles('dark', { backgroundColor: gray[800] }),
        variants: [{ props: { variant: 'outlined' }, style: { border: `1px solid ${(theme.vars || theme).palette.divider}`, boxShadow: 'none', background: 'hsl(0, 0%, 100%)', ...theme.applyStyles('dark', { background: alpha(gray[900], 0.4) }) } }],
      }),
    },
  },
